...
Button::Button(SDL_FRect rect, string textureId, string texturePath,string SFXId,string SFXPath, void(*callback)()) : GameObject(rect, textureId)
{
	sfxId = SFXId;
	TextureManager::Instance()->load(textureId, texturePath);
    SoundManager::Instance()->load(SFXId, SFXPath, SoundType::SFX);
...
void Button::onMouseClickButton()
{
	...
	SoundManager::Instance()->playSound(sfxId, 0);
